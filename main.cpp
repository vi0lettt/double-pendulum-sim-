...
vector<double> time_points;          // Временные точки
vector<double> theta1_values;        // Значения theta1
vector<double> theta2_values;        // Значения theta2
vector<double> omega1_values;        // Значения omega1
vector<double> omega2_values;        // Значения omega2
double current_time = 0.0;           // Текущее время симуляции
...
void save_data(const string& filename, const vector<double>& time_points, const vector<double>& theta1_values,
    const vector<double>& theta2_values, const vector<double>& omega1_values, const vector<double>& omega2_values) {
ofstream outfile(filename);
outfile << "Time,Theta1,Theta2,Omega1,Omega2\n";
for (size_t i = 0; i < time_points.size(); ++i) {
outfile << time_points[i] << ","
     << theta1_values[i] << ","
     << theta2_values[i] << ","
     << omega1_values[i] << ","
     << omega2_values[i] << "\n";
...
void display(int choise) {
    ...
    theta1_values.push_back(s.theta1);
    theta2_values.push_back(s.theta2);
    omega1_values.push_back(s.omega1);
    omega2_values.push_back(s.omega2);
...
int main() {
    {
        State s_rk4 = {M_PI / 2, M_PI / 2, 0.0, 0.0}; 
        vector<double> time_points_rk4;
        vector<double> theta1_values_rk4;
        vector<double> theta2_values_rk4;
        vector<double> omega1_values_rk4;
        vector<double> omega2_values_rk4;
        double current_time_rk4 = 0.0;

        while (current_time_rk4 < 4100.0) {
            time_points_rk4.push_back(current_time_rk4);
            theta1_values_rk4.push_back(s_rk4.theta1);
            theta2_values_rk4.push_back(s_rk4.theta2);
            omega1_values_rk4.push_back(s_rk4.omega1);
            omega2_values_rk4.push_back(s_rk4.omega2);

            runge_kutta4(s_rk4);
            current_time_rk4 += dt;
        }

        save_data("pendulum_data_RK4.csv", time_points_rk4, theta1_values_rk4, theta2_values_rk4, omega1_values_rk4, omega2_values_rk4);
    }

   
    {
        State s_dp8 = {M_PI / 2, M_PI / 2, 0.0, 0.0}; 
        vector<double> time_points_dp8;
        vector<double> theta1_values_dp8;
        vector<double> theta2_values_dp8;
        vector<double> omega1_values_dp8;
        vector<double> omega2_values_dp8;
        double current_time_dp8 = 0.0;

        while (current_time_dp8 < 4100.0) {
            time_points_dp8.push_back(current_time_dp8);
            theta1_values_dp8.push_back(s_dp8.theta1);
            theta2_values_dp8.push_back(s_dp8.theta2);
            omega1_values_dp8.push_back(s_dp8.omega1);
            omega2_values_dp8.push_back(s_dp8.omega2);

            dormand_prince_8(s_dp8);
            current_time_dp8 += dt;
        }

        save_data("pendulum_data_DP8.csv", time_points_dp8, theta1_values_dp8, theta2_values_dp8, omega1_values_dp8, omega2_values_dp8);
    }
